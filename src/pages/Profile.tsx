...
const Profile: React.FC = () => {
  ...
  const fetchPlayerStats = async () => {
    ...
    try {
      ...
      // Calculate level based on wins
      const { level, nextLevelWins, winsToNextLevel } = calculateLevel(gamesWon)
...
💎 Total Winnings: ${stats.totalWinnings.toFixed(4)} GOR

Join the dragon battles at DrawGor! 🔥

@Gorbagana_chain #DrawGor #DragonPower #BlockchainGaming #GORChain #CryptoGaming #DragonWarrior`
            ...
            <div className="dragon-card rounded-2xl p-8 dragon-shadow">
              ...
              <div className="space-y-2">
                ...
                <p className="text-amber-400 max-w-md mx-auto font-semibold">
                  Connect your wallet to view your dragon profile, battle statistics, and epic achievements
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-8 max-w-4xl mx-auto">
              ...
              <div className="max-w-md mx-auto">
                <div className="bg-white/20 rounded-full h-4 overflow-hidden">
                  <div 
                    ...
                    style={{ 
                      width: `${((stats.gamesWon - (stats.nextLevelWins - stats.winsToNextLevel - stats.gamesWon)) / stats.winsToNextLevel) * 100}%` 
                    }}
                  ></div>
                </div>
                <p className="text-white/70 text-sm mt-2 font-semibold">
                  {stats.gamesWon} / {stats.nextLevelWins} dragon victories
                </p>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="dragon-card rounded-xl p-6 text-center dragon-scale-hover border border-green-500/30">
            <Trophy className="h-12 w-12 text-green-400 mx-auto mb-3 dragon-wing" />
            <div className="text-4xl font-bold text-amber-300 mb-1 dragon-meme-text">{stats.gamesWon}</div>
            <div className="text-green-300 text-sm font-bold">🏆 DRAGON VICTORIES</div>
          </div>

          {/* Games Played */}
          <div className="dragon-card rounded-xl p-6 text-center dragon-scale-hover border border-blue-500/30">
            <Target className="h-12 w-12 text-blue-400 mx-auto mb-3 dragon-breath" />
            <div className="text-4xl font-bold text-amber-300 mb-1 dragon-meme-text">{stats.gamesPlayed}</div>
            <div className="text-blue-300 text-sm font-bold">⚔️ BATTLES FOUGHT</div>
          </div>

          {/* Win Rate */}
          <div className="dragon-card rounded-xl p-6 text-center dragon-scale-hover border border-purple-500/30">
            <TrendingUp className="h-12 w-12 text-purple-400 mx-auto mb-3 dragon-wing" />
            <div className="text-4xl font-bold text-amber-300 mb-1 dragon-meme-text">{stats.winRate.toFixed(1)}%</div>
            <div className="text-purple-300 text-sm font-bold">📊 DRAGON POWER</div>
          </div>

          {/* Total Winnings */}
          <div className="dragon-card rounded-xl p-6 text-center dragon-scale-hover border border-yellow-500/30">
            <Award className="h-12 w-12 text-yellow-400 mx-auto mb-3 dragon-breath" />
            <div className="text-4xl font-bold text-amber-300 mb-1 dragon-meme-text">{stats.totalWinnings.toFixed(4)}</div>
            <div className="text-yellow-300 text-sm font-bold">💎 DRAGON GOLD</div>
          </div>
        </div>
          ...
          {stats.totalUnclaimed > 0 && (
            <div className="mt-6 p-4 dragon-fire-bg rounded-lg text-center dragon-glow">
              <p className="text-black font-bold text-lg dragon-meme-text mb-2">
                🐉 You have {stats.totalUnclaimed.toFixed(4)} GOR in unclaimed dragon treasure!
              </p>
              <a
                href="/history"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className={`p-4 rounded-lg border text-center dragon-scale-hover transition-all ${
              ...
              <Trophy className={`h-10 w-10 mx-auto mb-2 ${
                stats.gamesWon >= 1 ? 'text-black dragon-wing' : 'text-gray-500'
              }`} />
              <div className={`text-sm font-bold dragon-meme-text ${
                stats.gamesWon >= 1 ? 'text-black' : 'text-gray-400'
              }`}>
                🥇 FIRST VICTORY
              </div>
            </div>
            ...
            <div className={`p-4 rounded-lg border text-center dragon-scale-hover transition-all ${
              ...
              <Zap className={`h-10 w-10 mx-auto mb-2 ${
                stats.gamesWon >= 5 ? 'text-black dragon-breath' : 'text-gray-500'
              }`} />
              <div className={`text-sm font-bold dragon-meme-text ${
                stats.gamesWon >= 5 ? 'text-black' : 'text-gray-400'
              }`}>
                ⚡ STREAK MASTER
              </div>
            </div>
            ...
            <div className={`p-4 rounded-lg border text-center dragon-scale-hover transition-all ${
              ...
              <Award className={`h-10 w-10 mx-auto mb-2 ${
                stats.gamesWon >= 10 ? 'text-black dragon-wing' : 'text-gray-500'
              }`} />
              <div className={`text-sm font-bold dragon-meme-text ${
                stats.gamesWon >= 10 ? 'text-black' : 'text-gray-400'
              }`}>
                🎰 HIGH ROLLER
              </div>
            </div>
            ...
            <div className={`p-4 rounded-lg border text-center dragon-scale-hover transition-all ${
              ...
              <Crown className={`h-10 w-10 mx-auto mb-2 ${
                stats.gamesWon >= 50 ? 'text-black dragon-breath' : 'text-gray-500'
              }`} />
              <div className={`text-sm font-bold dragon-meme-text ${
                stats.gamesWon >= 50 ? 'text-black' : 'text-gray-400'
              }`}>
                👑 DRAGON LEGEND
              </div>
            </div>
