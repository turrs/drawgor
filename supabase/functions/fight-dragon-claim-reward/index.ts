/*
# Fight Dragon Claim Reward Edge Function

This function handles prize distribution to Fight Dragon battle winners:
...
Deno.serve(async (req) => {
  ...
  try {
    console.log('🎁 Starting Fight Dragon reward claim process...')
    ...
    console.log(`🔍 Processing Fight Dragon reward claim for player: ${player_id}, wallet: ${wallet_address}`)

    // 1. Validate the player and check if reward is claimable
    console.log('🔍 Fetching Fight Dragon player data...')
    ...
    console.log('👤 Fight Dragon Player query result:', { 
      playerId: player?.id,
    ...
    if (playerError) {
      throw new Error(`Failed to fetch Fight Dragon player: ${playerError.message}`)
    }

    if (!player) {
      throw new Error('Fight Dragon player not found')
    }
    ...
    if (!player.is_winner) {
      throw new Error('Player is not a battle winner')
    }

    if (player.reward_claimed) {
      throw new Error(`Battle reward already claimed. Transaction hash: ${player.reward_transaction_hash || 'N/A'}`)
    }
    ...
    console.log(`✅ Valid Fight Dragon claim: ${player.prize_amount} GOR for ${wallet_address}`)
    ...
    console.log('🔒 Marking Fight Dragon reward as claimed in database to prevent double claims...')
    ...
    if (claimError) {
      throw new Error(`Failed to mark Fight Dragon reward as claimed: ${claimError.message}`)
    }

    console.log('✅ Fight Dragon reward marked as claimed in database')
    ...
    try {
      ...
      console.log(`📤 Fight Dragon Transfer details:`)
      ...
      console.log(`🎉 Fight Dragon reward claim completed successfully!`)
      console.log(`📊 Summary:`)
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: `Successfully sent ${player.prize_amount} GOR to your wallet from Fight Dragon battle`,
      ...
      if (rollbackError) {
        ...
        throw new Error(`Blockchain transaction failed AND database rollback failed. Manual intervention required for Fight Dragon player ${player_id}. Original error: ${blockchainError.message}`)
      }
    ...
    console.error('❌ Fight Dragon reward claim error:', error)
    ...
    if (error.message.includes('insufficient funds')) {
      userFriendlyMessage = 'Insufficient funds in the battle prize pool. Please contact support.'
    } else if (error.message.includes('already claimed')) {
      userFriendlyMessage = 'This battle reward has already been claimed.'
    } else if (error.message.includes('not a battle winner')) {
      userFriendlyMessage = 'You are not eligible for a reward in this battle.'
